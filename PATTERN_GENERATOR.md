...
This document describes the **reference app's** Pattern Generator, as the functional basis for ours.

Our QML port has since started.
`qml/windows/PatternAndMonitor.qml` hosts three panels matching the three sections above, plus the Monitor Viewer:

| Panel | File |
|---|---|
| Concentric | `qml/panels/ConcentricPanel.qml` |
| Stripline | `qml/panels/StriplinePanel.qml` |
| Chessboard | `qml/panels/ChessboardPanel.qml` |
| Monitor Viewer | `qml/panels/DpadMonitorViewer.qml` |

These are UI only.
No rendering engine, no JSON import/export, and no "update to monitor" exist yet, so every control emits a signal that nothing handles.
The Import and Export buttons in `ChessboardPanel` are worse than unhandled: they call signals that were never declared, so clicking them throws.
See `QML_LINT_DEBT.md`.

The improvement ideas above are still unimplemented, and are the reason to build the layer tables before the renderer.
